...
% Last Modified by GUIDE v2.5 18-Apr-2018 00:10:19
...
for k=1:3
    for j=1:w2
        for i=1:h2
            for l=1:8
            if(t<=len)
            bs(t)=mod(double(b(i,j,k)),2);
            b(i,j,k)=floor(double(b(i,j,k))/2);
            t=t+1;
            end
            end
        end
...
for i=1:h
    ...
    re=reshape(n,ht,wd,3);
    res=uint8(re);
    imwrite(res,'orig.jpg');
